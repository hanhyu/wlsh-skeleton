...
class Validation
{
     ...
     * @throws ValidateException
     */
    public static function validateIntIn($value, $valueList, $reason = null, $alias = 'Parameter'): float|int|string
    {
        if (is_array($valueList) === false || count($valueList) === 0)
            throw new ValidateException("验证器IntIn的参数格式错误, 必须提供整数的列表, 以逗号分隔");
     ...
     * @throws ValidateException
     */
    public static function validateIntNotIn($value, $valueList, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_array($valueList) === false || count($valueList) === 0)
            throw new ValidateException("验证器IntNotIn的参数格式错误, 必须提供整数的列表, 以逗号分隔");
     ...
     * @throws ValidateException
     */
    public static function validateStrEq($value, $equalsValue, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateStrNe($value, $equalsValue, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
            if ($value !== $equalsValue)
                return $value;
     ...
     * @throws ValidateException
     */
    public static function validateStrIn($value, $valueList, $reason = null, $alias = 'Parameter'): string
    {
        if (is_array($valueList) === false || count($valueList) === 0)
            throw new ValidateException("“{$alias}”参数的验证模版(StrIn:)格式错误, 必须提供可取值的列表");
     ...
     * @throws ValidateException
     */
    public static function validateStrNotIn($value, $valueList, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_array($valueList) === false || count($valueList) === 0)
            throw new ValidateException("“{$alias}”参数的验证模版(StrNotIn:)格式错误, 必须提供不可取的值的列表");
     ...
     * @throws ValidateException
     */
    public static function validateStrEqI($value, $equalsValue, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateStrNeI($value, $equalsValue, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
            if (strcasecmp($value, $equalsValue) !== 0)
                return $value;
     ...
     * @throws ValidateException
     */
    public static function validateStrInI($value, $valueList, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_array($valueList) === false || count($valueList) === 0)
            throw new ValidateException("“{$alias}”参数的验证模版(StrInI:)格式错误, 必须提供可取值的列表");
     ...
     * @throws ValidateException
     */
    public static function validateStrNotInI($value, $valueList, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_array($valueList) === false || count($valueList) === 0)
            throw new ValidateException("“{$alias}”参数的验证模版(StrNotInI:)格式错误, 必须提供不可取的值的列表");
     ...
     * @throws ValidateException
     */
    public static function validateStrLen($value, $length, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateStrLenGe($value, $min, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateStrLenLe($value, $max, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateStrLenGeLe($value, $min, $max, $reason = null, $alias = 'Parameter'): string
    {
        if ($min > $max)
            throw new ValidateException("“{$alias}”参数的验证模版StrLenGeLe格式错误, min不应该大于max");
     ...
     * @throws ValidateException
     */
    public static function validateByteLen($value, $length, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateByteLenGe($value, $min, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
            if (strlen($value) >= $min)
                return $value;
     ...
     * @throws ValidateException
     */
    public static function validateByteLenLe($value, $max, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
            if (strlen($value) <= $max)
                return $value;
     ...
     * @throws ValidateException
     */
    public static function validateByteLenGeLe($value, $min, $max, $reason = null, $alias = 'Parameter'): string
    {
        if ($min > $max)
            throw new ValidateException("“{$alias}”参数的验证模版ByteLenGeLe格式错误, min不应该大于max");
     ...
     * @throws ValidateException
     */
    public static function validateLetters($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateAlphabet($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
            if (@preg_match('/^[a-zA-Z]+$/', $value) === 1)
                return $value;
     ...
     * @throws ValidateException
     */
    public static function validateNumbers($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateDigits($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
            if (@preg_match('/^[0-9]+$/', $value) === 1)
                return $value;
     ...
     * @throws ValidateException
     */
    public static function validateLettersNumbers($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateNumeric($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
            if (@preg_match('/^-?[0-9.]+$/', $value) === 1) {
                $count = 0; //小数点的个数
     ...
     * @throws ValidateException
     */
    public static function validateVarName($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateEmail($value, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateUrl($value, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateIp($value, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateMac($value, $reason = null, $alias = 'Parameter'): string
    {
        if (is_string($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateRegexp(string $value, $regexp, $reason = null, string $alias = 'Parameter'): mixed
    {
        if (is_string($regexp) === false || $regexp === '')
            throw new ValidateException("“{$alias}”参数的验证模版(Regexp:)格式错误, 没有提供正则表达式");
        ...
        if (is_string($value)) {
            ...
            if ($result === 1)
                ...
                throw new ValidateException("“{$alias}”参数的正则表达式验证失败, 请检查正则表达式是否合法");
            $isTypeError = false;
        } else
            $isTypeError = true;
     ...
     * @throws ValidateException
     */
    public static function validateArr($value, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            $is = true;
     ...
     * @throws ValidateException
     */
    public static function validateArrLen($value, $length, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            $is = true;
            foreach ($value as $key => $val) {
     ...
     * @throws ValidateException
     */
    public static function validateArrLenGe($value, $min, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            $is = true;
            foreach ($value as $key => $val) {
     ...
     * @throws ValidateException
     */
    public static function validateArrLenLe($value, $max, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            $is = true;
            foreach ($value as $key => $val) {
     ...
     * @throws ValidateException
     */
    public static function validateArrLenGeLe($value, $min, $max, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            $is = true;
            foreach ($value as $key => $val) {
     ...
     * @throws ValidateException
     */
    public static function validateObj($value, $reason = null, $alias = 'Parameter'): mixed
    {
        if (is_array($value)) {
            $is = true;
            foreach ($value as $key => $val) {
    ...
    {
        if ($isFilesArray)
            throw new ValidateException("“{$alias}[$fileIndex]”上传失败(ERR=$err)");
        else
            throw new ValidateException("“{$alias}”上传失败(ERR=$err)");
    }

    /**
     * @throws ValidateException
     */
    public static function validateFile($value, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
     ...
     * @throws ValidateException
     */
    public static function validateFileImage($value, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            if (isset($value['name']) && isset($value['type']) &&
                isset($value['tmp_name']) && isset($value['error']) &&
     ...
     * @throws ValidateException
     */
    public static function validateFileVideo($value, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            if (isset($value['name']) && isset($value['type']) &&
                isset($value['tmp_name']) && isset($value['error']) &&
     ...
     * @throws ValidateException
     */
    public static function validateFileAudio($value, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            if (isset($value['name']) && isset($value['type']) &&
                isset($value['tmp_name']) && isset($value['error']) &&
     ...
     * @throws ValidateException
     */
    public static function validateFileMimes($value, $mimes, $originMimesString = null, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            if (isset($value['name']) && isset($value['type']) &&
                isset($value['tmp_name']) && isset($value['error']) &&
                ...
                for ($i = 0, $count = count($errors); $i < $count; $i++) {
                    ...
                    if ($error === UPLOAD_ERR_OK) {
                        ...
                        foreach ($mimes as $mime) {
                            // mime中必定有斜杠
                            if (str_starts_with($mime, '/')) // 斜杠在开头, 需要完全尾部匹配
                            {
                            ...
                            {
                                if (str_starts_with($type, $mime)) // 匹配mime
                                {
                                    $match = true;
                                    break;
                                }
     ...
     * @throws ValidateException
     */
    public static function validateFileMaxSize($value, $maxSize, $originSizeString = null, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            if (isset($value['name']) && isset($value['type']) &&
                isset($value['tmp_name']) && isset($value['error']) &&
     ...
     * @throws ValidateException
     */
    public static function validateFileMinSize($value, $minSize, $originSizeString = null, $reason = null, $alias = 'Parameter'): array
    {
        if (is_array($value)) {
            if (isset($value['name']) && isset($value['type']) &&
                isset($value['tmp_name']) && isset($value['error']) &&
    ...
    // 当前语言代码
    static protected string $langCode = '';
...
//        ],
     ...
     * @throws ValidateException
     */
    private static function arrayMergeRecursive($arr1, $arr2): array
    {
        if ($arr1 === null)
            $arr = [];
        else if (!is_array($arr1))
            throw new ValidateException(__CLASS__ . '::' . "() 参数 \$arr1 只能是数组或null");
        else
            $arr = [] + $arr1;
        ...
        if ($arr2 === null)
            ...
            throw new ValidateException(__CLASS__ . '::' . "() 参数 \$arr2 只能是数组或null");
...
//        "en-us" => [],
    ...
    // 所有验证器格式示例
    static private array $sampleFormats = [
        // 整型（不提供length检测,因为负数的符号位会让人混乱, 可以用大于小于比较来做到这一点）
        'Int'            => 'Int',
        'IntEq'          => 'IntEq:100',
        'IntNe'          => 'IntNe:100',
     ...
     */
    private static function _compileValidationRule($validationRule, $alias): array
    {
        if (is_string($validationRule) === false)
            throw new ValidateException("编译验证规则失败: 验证规则必须是字符串");;
        ...
        for ($i = 0; $i < $segCount;) {
            $segment = $segments[$i];
            $i++;
            if (str_starts_with($segment, 'Regexp:')) // 是正则表达式
            {
                if (strpos($segment, '/') !== 7) // 非法的正则表达. 合法的必须首尾加/
                    throw new ValidateException("正则表达式验证器Regexp格式错误. 正确的格式是 Regexp:/xxxx/");
                ...
                if ($pos === false) {
                    if ($segment === 'Required') {
                        if (count($validators) > $countOfIfs) {
                            throw new ValidateException("Required只能出现在验证规则的开头（IfXxx后面）");
                        }
                        $required = true;
                    } else {
                        ...
                        if (method_exists(static::class, $methodName) === false)
                            throw new ValidateException("未知的验证器\"{$validatorName}\"");

                        if (str_starts_with($validatorName, 'Custom')) {
                            if (strlen($validatorName) == 6)
                                throw new ValidateException("自定义验证器必须以\"Custom\"开头，但不能是\"Custom\"");
                            $class         = new \ReflectionClass(static::class);
                            $method        = $class->getMethod($methodName);
                            $reflectParams = $method->getParameters();
                            $pCount        = count($reflectParams) - 3;
                            if ($pCount < 0) {
                                throw new ValidateException("自定义验证器\"{$validatorName}\"的实现方法 $methodName() 应该至少有3个参数");
                            } else if ($pCount > 0) {
                                throw new ValidateException("自定义验证器\"{$validatorName}\"应该有 $pCount 个参数");
                            }
                        }
                    ...
                    if (strlen($validatorName) === 0) {
                        throw new ValidateException("“{$segment}”中的':'号前面没有验证器");
                    }
                    $validatorInfo = null;
                    switch ($validatorName) {
                        ...
                        case 'IfIntLe':
                            if (count($validators) > $countOfIfs)
                                throw new ValidateException("条件验证器 IfXxx 只能出现在验证规则的开头");
...
//                            break;
                        ...
                        default:
                            ...
                            if (str_starts_with($validatorName, 'Custom') &&
                                ...
                                if ($pCount < 0) {
                                    ...
                                    throw new ValidateException("自定义验证器\"{$validatorName}\"应该没有参数");
                                } else {
                                    ...
                                    if (method_exists($className, $parserMethod)) { // 如果实现了参数解析方法
                                        ...
                                        if (!is_array($params) || count($params) === 0 || $pCount != count($params))
                                            throw new ValidateException("自定义验证器\"{$validatorName}\"的参数解析方法 $parserMethod() 应该返回含有 $pCount 个参数的数组");
                                    } else {
                                        $params = explode(',', $p);
    ...
    private static function _throwFormatError($validatorName)
    {
        ...
        if ($sampleFormat === null)
            throw new ValidateException("验证器{$validatorName}格式错误");
        throw new ValidateException("验证器{$validatorName}格式错误, 正确的格式是: $sampleFormat");
    }
     ...
     */
    private static function _parseIntArray($value): array|bool
    {
        $vals = explode(',', $value);
        $ints = [];
        foreach ($vals as $val) {
     ...
     */
    private static function _parseStringArray($value): array|bool
    {
        if (strlen($value)) {
            $vals = explode(',', $value);
            if ($vals === false)
...
//            $vals = array_unique($vals); // 不需要去重, 不影响结果. 程序员不应该写出重复的字符串
    ...
    private static function _parseSizeString($value): ?int
    {
        $value   = mb_strtolower($value);
        $matches = [];
...
//                $size *= 1048576*1024;
     ...
     */
    private static function _parseDateString($value): ?int
    {
        $result = @preg_match('/^\d\d\d\d-\d?\d-\d?\d$/', $value);
        if ($result !== 1)
            return null;
     ...
     */
    private static function _parseTwoDateStrings($value): ?array
    {
     ...
     */
    private static function _parseDateTimeString($value): ?int
    {
        $result = @preg_match('/^\d\d\d\d-\d?\d-\d?\d \d\d:\d\d:\d\d$/', $value);
        if ($result !== 1)
            return null;
     ...
     */
    private static function _parseTwoDateTimeStrings($value): ?array
    {
        $dateStrings = explode(',', $value);
        if (count($dateStrings) !== 2)
            return null;
     ...
     * @throws ValidateException
     */
    private static function _parseMimesArray($value): array|bool
    {
        $mimes   = explode(',', $value);
        $strings = [];
        foreach ($mimes as $mime) {
            ...
            {
                if ($mime === '*')
                    throw new ValidateException("无效的MIME“{$mime}”");
                $mime = "/$mime";
     ...
     * @throws ValidateException
     */
    private static function _parseIfXxxWith1Param1Int($paramstr, $validatorName): false|array
    {
     ...
     * @throws ValidateException
     */
    private static function _parseIfXxxWith1Param1Str($paramstr, $validatorName): false|array
    {
        $params = explode(',', $paramstr);
        if (count($params) != 2)
            return false;
...
//        $value = $params[1];
        if (strlen($varName) == 0) // 简单检测
            throw new ValidateException("“$validatorName:{$paramstr}”中“{${$validatorName}}”后面必须是一个参数（变量）名，实际上却是空串");
        return $params;
    }
     ...
     * @throws ValidateException
     */
    private static function _parseIfXxxWith1Param($paramstr): false|string
    {
        $params = explode(',', $paramstr);
        if (count($params) != 1)
            return false;
...
//        $varName = $params[0];
     ...
     */
    private static function _parseIfXxxWith1ParamMultiStrings($paramstr, $validatorName): false|array
    {
        $params = explode(',', $paramstr);
        $c      = count($params);
        if ($c < 2)
     ...
     * @throws ValidateException
     */
    private static function _parseIfXxxWith1ParamMultiInts($paramstr, $validatorName): false|array
    {
        $params = explode(',', $paramstr);
        $c      = count($params);
        if ($c < 2)
     ...
     * @throws ValidateException|\ReflectionException
     */
    public static function validateValue($value, $validationRule, $alias = 'Parameter', $ignoreRequired = false, $originParams = [], $siblings = []): mixed
    {
        if (is_array($validationRule)) {
            ...
            throw new ValidateException(self::class . '::' . __FUNCTION__ . "(): \$validationRule必须是字符串或字符串数组");
...
//                    echo "\n".json_encode($validatorInfo)."\n";
                    ...
                    if (method_exists(self::class, $method) === false)
                        throw new ValidateException("找不到条件判断{${$ifName}}的验证方法");
                    ...
                    } else // 绝对路径
                    {
                        ...
                        if ($asterisksCount > 0) {
                            throw new ValidateException("IfXxx中的条件参数“{$varkeypath}”中不得包含*号");
                        }
                    ...
                    if ($ignoreRequired) {// 这是增量更新
                        if ($value !== null) {// 如果参数存在，则其依赖的条件参数也必须存在
                            if ($ifParamValue === null && // 依赖的条件参数不存在
                                ...
                                throw new ValidateException("必须提供条件参数“{$varkeypath}”，因为“{$alias}”的验证依赖它");
                            }
...
//                    }
     ...
     * @throws ValidateException
     */
    private static function _compileKeypath(string $keypath, &$asterisksCount): array
    {
        if (strlen($keypath) === 0)
            throw new ValidateException('参数$validations中包含空的参数名称');

        if (@preg_match('/^[a-zA-Z0-9_.\[\]*]+$/', $keypath) !== 1)
            throw new ValidateException("非法的参数名称“{$keypath}”");
        ...
        foreach ($keys as $key) {
            if (strlen($key) === 0)
                throw new ValidateException("“{$keypath}”中包含空的参数名称");

            $i = strpos($key, '[');
            if ($i === false) // 普通的key
            {
                if (str_contains($key, '*'))
                    throw new ValidateException("“{$keypath}”中'*'号只能处于方括号[]中");
                if (str_contains($key, ']'))
                    throw new ValidateException("“{$key}”中包含了非法的']'号");
                if (@preg_match('/^[0-9]/', $key) === 1) {
                    if (count($keys) === 1)
                        throw new ValidateException("参数名称“{$keypath}”不得以数字开头");
                    else
                        throw new ValidateException("“{$keypath}”中包含了以数字开头的参数名称“{$key}”");
                }
                $filteredKeys[] = $key;
                continue;
            } else if ($i === 0) {
                throw new ValidateException("“{$keypath}”中'['号前面没有参数名称");
            } else {
                $j = strpos($key, ']');
                if ($j === false)
                    throw new ValidateException("“{$key}”中的'['号之后缺少']'");
                if ($i > $j)
                    throw new ValidateException("“{$key}”中'[', ']'顺序颠倒了");
                ...
                if (@preg_match('/^[0-9]/', $varName) === 1)
                    throw new ValidateException("“{$keypath}”中包含了以数字开头的参数名称“{$varName}”");
                ...
                if ($index === '*') {
                    ...
                    throw new ValidateException("“{$key}”中的方括号[]之间只能包含'*'号或数字");
                ...
                while ($j < $len - 1) {
                    ...
                    $i = strpos($key, '[', $j);
                    if ($i !== $j)
                        throw new ValidateException("“{$key}”中的“[$index]”之后包含非法字符");
                    $j = strpos($key, ']', $i);
                    if ($j === false)
                        throw new ValidateException("“{$key}”中的'['号之后缺少']'");
     ...
     * @param $ignoreRequired bool 是否忽略所有的Required检测子
     * @return array
     * @throws ValidateException 验证不通过会抛出异常
     */
    public static function validate(array $params, array $validations, bool $ignoreRequired = false): array
    {
        if (is_array($params) === false)
            throw new ValidateException(self::class . '::' . __FUNCTION__ . "(): \$params必须是数组");
...
//            echo json_encode($cachedKeyValues, JSON_PRETTY_PRINT);
     ...
     * @throws ValidateException
     */
    private static function getParamValueForIf($params, $keys, &$ancestorExist = null): mixed
    {
        $keysCount = count($keys);
     ...
     * @throws ValidateException
     */
    private static function _validate($params, $keys, $keysCount, $validationRule, $keyPrefix = '', $ignoreRequired = false, &$cachedKeyValues = null): void
    {
        $keyPath  = $keyPrefix;
        $siblings = $params;
        $value    = $params;
